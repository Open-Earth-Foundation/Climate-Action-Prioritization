def return_prompt(action, city):
     # Generate the prioritization prompt
        prompt = f"""
                        You are a climate action expert, tasked to prioritize and recommend the better of two actions for a city based on the following guidelines:
                        
                        ### Guidelines for Action Prioritization in the order of importance:
                        1. **Emissions Reduction:** Actions that achieve significant greenhouse gas (GHG) emissions reduction should rank higher, especially those targeting the city's largest emission sectors. This is the most important factor in the scoring. Actions that significantly reduce emissions in the city's most emission-intensive sector should always be scored better.
                        ...
                        ### Instructions:
                        - Based on the rules, evaluate the two actions provided.
                        - Consider both qualitative and quantitative aspects of the actions.
                        - Remember that data is provided in the order of importance and you should consider emissions and sectors first.
                        - Determine which of the two actions is better.
                        - Provide a detailed explanation for why the chosen action was prioritized.

                        ### Action Data (Two Actions):
                        {str(action)}
...
def return_prompt1(action, city):
    """
